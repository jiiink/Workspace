...
class BST {
public:
   BST(BstNode<Type> *init = 0) {root = init;}; //   BST& operator=(const BST&);
...
main() {
   ...
   cout << "\n" << m.Insert(a);
   cout << "\n" << m.Insert(b); //   m.treeprint();
   cout << "\n" << m.Insert(c); //   m.treeprint();
   cout << "\n" << m.Insert(d); //   m.treeprint();
   cout << "\n" << m.Insert(e); //   m.treeprint();
   cout << "\n" << m.Insert(f); //   m.treeprint();
   cout << "\n" << m.Insert(g); //   m.treeprint();
   cout << "\n" << m.Insert(h); //   m.treeprint();
   cout << "\n" << m.Insert(i); //   m.treeprint();
   cout << "\n" << m.Insert(j);
   m.treeprint();

   BST<int> n, o;
   Element<int> el;
   cout << "Split Tree with node=11 \n" ;

   // tree m을 data=11을 중심으로 쪼개서 n과 o로 분리하라.
   cout << (m.Split(11, n, o, el))->key << endl;

    cout << "Tree after Split node=11 \n" ;
    n.treeprint();
    cout << "Tree after Split node=11 \n" ;
    o.treeprint();
