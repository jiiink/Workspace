// Combining STL components with inheritance and virtual functions
#include "shape.h"
#include <vector>
#include <algorithm>
#include <iostream>
...
int main() { // First part is same as in Stroustrup's book:
  screen_init();
...
// Use STL-provided iterator to move all points horizontally 20 units:
  vector<shape*>::iterator i;
  for (i = vs.begin(); i != vs.end(); ++i)
    (*i)->move(20, 0);
  shape_refresh();

// Demonstrate use of STL generic sorting algorithm with objects in a class hierarchy:
  outputWestX(vs);
