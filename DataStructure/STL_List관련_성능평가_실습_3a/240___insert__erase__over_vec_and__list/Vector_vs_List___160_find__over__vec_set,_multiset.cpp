...
int main() {
    ...
    auto tbegin = time.now();
        for(int i=0; i< N ; ++i) {
            ...
            ex4.insert   ( simhash2(i, 100000) ) ;
        }
    auto tend = time.now();
    diff_time = tend - tbegin;
    cout << "Number of Steps = " << N << endl ;
    cout << "Time set, vector 데이터 만들기 : " << diff_time.count() << endl;
    ...
    tend = time.now();
    diff_time = tend - tbegin;
    cout << "Time for sorted multiset.find()   : " << diff_time.count() << endl;
