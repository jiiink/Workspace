...
int main() {
    ...
    unordered_map   <int,int>      ex9 ; // 2022년 12월에 추가
    vector          <int>          Query;
    ...
    tbegin = time.now();
        for(int i=0; i< N ; ++i) {
            ...
            ex7.insert   ( rval ) ;
            ex8[i]   =     rval   ;   // map[ ]
            ex9[i]   =     rval   ;   // unordered_map[]

            Query.push_back( i ) ;    // find( )에서 검색에 사용할 데이터를 미리 준비
        }
    ...
    tbegin = time.now();
           for(int i=0; i< N/D ; ++i) find( ex1.begin(), ex1.end(), Query[i]) ;
    tend = time.now();
    diff_time = tend - tbegin;
    ...
    tbegin = time.now();
           for(int i=0; i< N/D ; ++i) lower_bound( ex1.begin(), ex1.end(), Query[i]) ;
    tend = time.now();
    diff_time = tend - tbegin;
    cout << "> lower_bound( on sorted vector[] ) : " << diff_time.count() << endl;

    tbegin = time.now();
           for(int i=0; i< N/D ; ++i) binary_search( ex1.begin(), ex1.end(), Query[i]) ;
    tend = time.now();
    diff_time = tend - tbegin;
    cout << "> binary_search( sorted vector[] )  : " << diff_time.count() << endl;

    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) find( ex3.begin(), ex3.end(), Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    ...
    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) find( ex5.begin(), ex5.end(), Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    ...
    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) binary_search( ex5.begin(), ex5.end(), Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    ...
    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) find( ex6.begin(), ex6.end(), Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    ...
    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) ex2.find( Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    cout << "> Time for set.find()             : " << diff_time.count() << endl;

    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) ex4.find( Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    cout << "> Time for multiset.find()        : " << diff_time.count() << endl;

    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) ex7.find( Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    cout << "> Time for unordered_set.find( )   : " << diff_time.count() << endl;

    tbegin = time.now();
          for(int i=0; i< N/D ; ++i) find( ex7.begin(), ex7.end(), Query[i] ) ;
    tend = time.now();
    diff_time = tend - tbegin;
    ...
    fail=0 ;
          for(int i=0; i< N/D ; ++i)
            if( ex9.find(Query[i]) != ex9.end() ) { fail++ ;   }
    tend = time.now();
    diff_time = tend - tbegin;
    cout << "> unordered_map ex9.find( Q )    : " << diff_time.count() << endl;
    return 0;
